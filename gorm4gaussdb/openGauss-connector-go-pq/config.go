...
var gSocketTimeout time.Duration
var gConnectTimeout time.Duration
...
func ParseConfig(connString string) (*Config, *DistConfig, error) {
	...
	if connectTimeoutSetting, present := settings["connect_timeout"]; present {
		...
		hasConnSettings = true
	}

	if connTCPRetrysSetting, present := settings["tcp_syn_retries"]; present {
		tcpRetryCount, err := parseconnTCPRetrys(connTCPRetrysSetting)
		if err != nil {
			return nil, nil, &parseConfigError{connString: connString, msg: "invalid tcp_syn_retries", err: err}
		}
		config.tcp_syn_retries = tcpRetryCount
		hasConnSettings = true
	}
	if socketTimeoutSetting, present := settings["socketTimeout"]; present {
		socketTimeout, err := parseConnectTimeoutSetting(socketTimeoutSetting)
		if err != nil {
			return nil, nil, &parseConfigError{connString: connString, msg: "invalid socketTimeout", err: err}
		}
		gSocketTimeout = socketTimeout
	}

	if hasConnSettings {
		config.DialFunc = makeConnectTimeoutDialFunc(config.ConnectTimeout, config.tcp_syn_retries)
	} else {
...
func parseconnTCPRetrys(s string) (int64, error) {
	intBase := 10
	intBitSize := 64
	retryTimes, err := strconv.ParseInt(s, intBase, intBitSize)
	if err != nil {
		return 0, err
	}
	var retryTimesMin int64 = 0
	var retryTimesMax int64 = 127
	if retryTimes <= retryTimesMin || retryTimes > retryTimesMax {
		return 0, errors.New("Invalid tcp_syn_retries, the value of tcp_syn_retries must be within the range of 1 to 127")
	}
	return retryTimes, nil
}

func makeConnectTimeoutDialFunc(timeout time.Duration, retrytimes int64) DialFunc {
	d := makeDefaultDialer()
	d.Timeout = timeout
	if retrytimes != 0 {
		d.Control = setSocketParameter(retrytimes)
	}
